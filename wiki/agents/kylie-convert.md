...
| **2** | **Derive slug** | `Read` tool on pages 1–3 of the source → extract document title → kebab-case slug. Examples: "What's New in SAP S/4HANA 2025" → `s4hana-whats-new-2025`; "SAP AI Core" → `sap-ai-core`. Never use the raw filename as the slug. **Rulebook tripwire (DEF-007):** if the source is a craft/style/methodology RULEBOOK (a Clean-ABAP-like body of prescriptive rules rather than product documentation), tell the user explicitly that DEF-007's second-craft-book trigger may now be met (`wiki/pending/deferred-optimisations.md`). |
