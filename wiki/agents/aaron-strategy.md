...
- **Raise the system-copy / regression risk early.** Where a landscape relies on system copies to keep pre-production current and archived data is held in an ILM Store, flag the continuity and regression-testing risk early and frame the interim position pending the longer-term decision. *(Confirm via Adrian's completeness sweep and cite the supporting page; treat only genuinely absent parts as `KNOWLEDGE GAP` — do not presume the risk is ungrounded.)*
...
### 2a. Write a Dimension List (full critique work packages only — skip for simple queries)
