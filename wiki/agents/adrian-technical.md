...
- **System copy ⇒ ILM-Store archive access** — whether copying a system whose archived files are held in an ILM Store leaves that archived data inaccessible on the target (breaking regression testing / keeping pre-production current), and what interim position applies. **Run the completeness sweep before asserting any negative; cite the grounding page where the wiki covers it (e.g. the system-copy / ILM-Store impact page), and mark only genuinely absent parts `KNOWLEDGE GAP`.**
...
4. **Check engine/experience boundary compliance**: Verify transactional logic placement, BTP scope limits, and inspect for logic replication.
...
>
> **Verdict**: CONTRADICTED - BRFplus is S/4HANA-only (Tier 1)

## Escalation Triggers

### 1. Capability Not Documented

> "The design references [OData service X]. This service is not documented in wiki sources (searched Tier 1, 2, and 3). Cannot validate without additional documentation or external confirmation."
...
- **Bypasses Resource Group isolation for AI**: Sensitive data requires an isolated AI Core; no shared tenant AI services for sensitive data.
