...
The orchestrator (main thread, Alex) spawns Dana immediately after **Anja finishes** writing pages and rebuilding the index. Anja no longer has reader sub-agents of her own to finish waiting on — for multi-file sources, Alex fans out the readers directly and Anja is invoked once, purely to consolidate and write (see `wiki/agents/alex-master.md` → "Multi-File Reader Fan-Out"). Dana also backstops the `Stop`/`SubagentStop` hook (`scripts/hooks/rebuild-and-verify.sh`), which runs `validate_wiki.py` automatically and blocks "done" on failure.
...
- **Flagged-pages scope on re-verification rounds (2+).** On round 1 Dana reviews all new and changed pages. On any re-verification round, if Alex supplies a `flagged:` list (the pages Dana flagged the previous round), re-read **only** those pages plus the registries they touch — the pages that already passed round 1 do not need a fresh semantic read. This is purely a narrowing of the semantic re-read: the mechanical `validate_wiki.py` run still covers the **whole corpus** every round and backstops everything unflagged, so nothing is skipped mechanically. If no `flagged:` list is supplied, fall back to re-reading all changed pages (safe default).
