...
- **Cross-cluster links**: Flag interface boundaries crossing cluster domains using explicit `[?INTEGRATION]` tokens embedded in the page body (harvested by `build_index.py` into `wiki/pending/index-report.md` for Sarah). **Never edit `wiki/pending/cross-links.md` directly.** If a batch produces finished cross-link table rows rather than in-page markers, write them as an inbox drop — create `wiki/pending/cross-links-inbox/<YYYY-MM-DD>-<source-id>.md` with one `| `-prefixed row per marker (matching the Active Pending Queue columns); Sarah's `resolve links` sweep consolidates them under `ops.lock` (OPT-006, see `wiki/operations.md` → Multi-session operation).
...
- **trm-pscd-core pages**: apply the "1. SAP TRM/PSCD Page Extension" metadata block from `wiki/schema.md` (mandatory — 4 fields: T-Codes, IMG Path, Process Area, S/4HANA Status).
...
- **Anja does not spawn Dana.** After writing pages, updating registries, and rebuilding the index, Anja's final step is to **report completion back to Alex** (see Ingest Report Format below). Alex (main thread) then spawns Dana directly: `Agent(subagent_type="dana-validator", prompt="@dana verify [source-name] ingestion — check all new and changed pages")`.
- If Dana returns mechanical ERRORS, Alex re-invokes Anja with the flagged pages for rework; Anja fixes them, re-runs `build_index.py`, and reports back to Alex for re-verification. Loop up to **3 rounds**, then Alex escalates to the human with the outstanding report.
- **Semantic findings are advisory** → Alex routes them to `@sarah` as proposals; they do not block completion.
...
> **Multi-file sources**: this protocol runs once per batch, not once per file. Anja is invoked a single time with all reader reports already attached to the prompt (see "Multi-File Ingestion (Alex Fans Out, Anja Consolidates)" above); Steps 1–4 below describe single-file ingestion, and are effectively already done by the readers for a multi-file batch — Anja begins at Step 5 (Create wiki pages) for those sources.
...
Before Alex dispatches readers (or before Anja begins single-file extraction), scan `wiki/sources.md` and the target cluster registry to determine prior coverage. For multi-file sources this scan is run once by Alex, ahead of the fan-out, and the resulting ALREADY list is passed to both the readers and to Anja's consolidation invocation — see "Who Builds the ALREADY List" below.
...
- **Rulebook tripwire (DEF-007)**: if the source is a craft/style/methodology RULEBOOK (a Clean-ABAP-like body of prescriptive rules rather than product documentation), tell the user explicitly that DEF-007's second-craft-book trigger may now be met (`wiki/pending/deferred-optimisations.md`).
...
**Who builds the ALREADY list**: for multi-file sources, Alex builds it (a single Read/Grep pass over `wiki/sources.md` and the target cluster registry, done once, before any reader is dispatched — not a sub-agent spawn). For single-file sources, Anja still builds it herself as part of her own single invocation, exactly as today. See `wiki/agents/alex-master.md` → "Multi-File Reader Fan-Out" for the Alex-side procedure.
...
**Paul write-back blocks** (`@anja ingest paul-writebacks`, relayed by Alex per `wiki/agents/alex-master.md` → "Paul Write-Back Relay") are a recognised source type feeding this same new/overlap/synonym classification — not a parallel mechanism. Each block already states its own routing (general cluster vs `client-dev-context`) and provenance (`[MCP] <SYSTEM> <date>` / `[SAP] <url>`); Anja classifies the underlying fact against existing pages exactly as for any other source, and for `client-dev-context` pages carries the block's `client:`/`system:` frontmatter values through unchanged.

### Multi-File Ingestion (Alex Fans Out, Anja Consolidates)

For multi-file sources, Anja does not dispatch reader sub-agents herself. **Alex (main thread) dispatches one read-only reader sub-agent per file directly**, in parallel, exactly as Alex does for Aaron/Adrian (see `wiki/agents/alex-master.md` → "Multi-File Reader Fan-Out"). Alex collects all N reader reports within its own continuously-live turn, then invokes Anja **once**, passing all N reader reports plus the ALREADY list in a single prompt.

Anja's job on a multi-file invocation is purely to **dedup, route, and write centrally**:

- Dedup each incoming page spec against the ALREADY list and each other (see Delta Mode above for the new/overlap/synonym classification).
- Route pages to the correct cluster (e.g. btp-platform vs btp-ai) using the same routing rules as single-file ingestion.
- Write all pages, update cluster registries, and rebuild the index (Steps 5–7 of the Operational Steps table, run once across the whole batch rather than once per file).
- Anja never calls the Agent tool. She has no reader-dispatch or Dana-dispatch responsibility of her own — both belong to Alex (see "Post-Ingestion Verification (Dana)" above).

Reader agents must not write to the wiki directory — they return structured page specs only (see Reader Return Format below); Alex passes their returns through to Anja verbatim.
...
**Reported to**: Alex (main thread) — Alex spawns Dana next.
**Source**: [filename]
**Cluster**: [cluster-name]
