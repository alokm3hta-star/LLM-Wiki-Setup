...
- [x] Clear explanation
- [ ] Card conformance: rule IDs cited match the dev card (core paul-dev-card.md + packs under wiki/agents/paul-card-packs/) (Paul write-back pages only)
...
**Card-conformance logging (DEF-008 observability).** When the "Card conformance" check fails on a Paul write-back page — a cited rule ID that the delivered code did not satisfy — log one row per finding:
`python3 scripts/append_row.py wiki/log.md "| <UTC-ISO> | audit-finding | card-conformance-miss | <rule-id> <object/page> |"`
`check_def_triggers.py` counts `card-conformance-miss` rows for the DEF-008 second trigger.
...
0. **Consolidate the inbox first.** Fold any writer drop-files (`wiki/pending/cross-links-inbox/*.md`) into the Active Pending Queue before working it, under the ops lock:
   `python3 scripts/with_lock.py ops.lock -- python3 scripts/consolidate_crosslinks.py`
   This is the sole editor of `cross-links.md`'s row set; writers only ever create inbox drops (OPT-006, see `wiki/operations.md` → Multi-session operation). Then work the table as below.
...
- `@sarah approve-all`: Enumerate the queue via `grep -l 'status: pending' wiki/pending/proposals/SP-*.md` (one file per proposal; status lives in that file's own frontmatter — there is no shared table to edit). Before executing the batch, output a one-line risk flag per proposal in the queue:
  ...
  Proposals flagged LOW or MEDIUM proceed automatically. On execution, edit that proposal's own file: set its frontmatter `status:` to `executed` (or `deferred`/`rejected` as applicable) — never a shared table row. This step cannot be skipped.

### Lesson-capture execution

When `@sarah approve-all` encounters a proposal of Type `Lesson capture` (produced by `@alex learn` or the reflection loop), execute it as follows:

- **Risk-flag** as for any proposal: LOW = a correction grounded in a cited wiki page, or a pure preference/orthography/behaviour rule; MEDIUM = a single user correction with no independent corroboration; **HIGH — HOLD** = the lesson embeds an unsourced SAP factual claim (a T-code, table, IMG path, or API asserted as fact). Hold HIGH items for the user. A lesson may encode *how an agent should behave*; it must never introduce a new ungrounded SAP fact.
- **Execute** on approval by writing to the `Target`:
  - `wiki/agents/lessons/<agent>.md` — append a dated bullet under `## Lessons`: `- YYYY-MM-DD [SP-NNN, approved]: <lesson>. Trigger: <when it applies>. Origin: <what prompted it>.` Bump the file's `updated:` field and increment its row count in `wiki/agents/lessons/_index.md`.
  - `wiki/profile/engagement.md` — edit the relevant section (Consultant / Active engagement / Standing preferences / Orchestration preferences); replace a placeholder rather than duplicating, and bump `updated:`.
- **Reconcile**: run `scripts/build_index.py` (lesson and profile files sit outside `wiki/pages/**`, so entity/page counts are unaffected; the run reconciles the pending-proposals count), then set that proposal file's frontmatter `status:` to `executed`.
- **Consolidate periodically**: when a lesson file grows large or accumulates near-duplicates, run a merge/prune pass (the `anthropic-skills:consolidate-memory` pattern) against a size budget — combine overlapping lessons, drop superseded ones, preserve provenance. Lessons are context, not knowledge; keep them short and never let one contradict a cited wiki fact.
...
**Suggested research sources**: [SAP Help Portal sections, SAP Community, etc.]
...
2. **`pending_research/_index.md`** — remove the brief's row from the active Research Briefs table and move the closed row plus any sweep narrative to `wiki/archive/<quarter>/research-briefs-closed.md` in the **same pass**; correct the active count in the Gap Summary prose. `_index.md` carries **active briefs only** — it is read at every session boot, so closure history left in it is a per-session token tax (OPT-010). Do not accumulate a "Closed — Covered" ledger inside `_index.md`; that ledger lives in the archive file.
