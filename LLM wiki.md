...
| **4**    | —                    | Unmapped concepts absent from indexes.         | —                      | `KNOWLEDGE GAP: [topic] not covered`          |
...
8. Alex prints the unified design critique block to the human user console interface.
...
- All Contract Accounts Receivable and Payable transaction postings, BRFplus business calculation rules, tax calculations, and FPF financial form routines must remain natively within the S/4HANA PCE engine core; there are no exceptions.
