...
from wikilib import atomic_write, count_status_rows
...
def _count_status_rows(relpath, statuses):
    """Count table rows whose LAST cell starts with one of `statuses` (e.g. 'pending').

    Delegates to wikilib.count_status_rows, which tolerates annotated cells like
    'pending - awaiting Sarah' and 'resolved 2026-06-21 [[page]]' — the real-world
    free-text status vocabulary in these files (see OPT-008).
    """
    return count_status_rows(ROOT / relpath, statuses)
...
def reconcile_counts(per_cluster, total, entity_total, cluster_total):
    ...
    p_pending = sum(1 for f in prop_dir.glob("SP-*.md")
                    if re.search(r"^status:\s*pending\b", f.read_text(errors="ignore"),
                                 re.MULTILINE)) if prop_dir.exists() else 0
