...
PROPOSALS_DIR = ROOT / "wiki" / "pending" / "proposals"
STATE = ROOT / "wiki" / "state.md"
LOG = ROOT / "wiki" / "log.md"
CROSS_LINKS = ROOT / "wiki" / "pending" / "cross-links.md"
GAP_LOG = ROOT / "wiki" / "pending" / "gap-log.md"
RETRIEVAL_LOG = ROOT / "wiki" / "pending" / "retrieval-log.md"
...
# gap-log / retrieval-log rows lead with a plain date (`| 2026-06-04 |`) or, for
# OPT-021 standardised retrieval rows, an ISO datetime (`| 2026-07-05T…`); match both.
LEDGER_ROW_RE = re.compile(r"^\| (\d{4})-(\d{2})-(\d{2})[ T]")
...
def split_proposals():
    ...
    whose status (last `|`-separated cell) does NOT start with executed/deferred.
    Uses startswith (not exact match) because real rows carry annotations, e.g.
    "executed — Sarah approve-all batch, 2026-07-02" (same fix as OPT-008; mirrors
    the already-correct startswith("resolved") model in split_cross_links below).
    """
    ...
    for line in lines:
        ...
        if in_queue and line.startswith("| SP-"):
            ...
            if status.startswith("executed") or status.startswith("deferred"):
...
def split_dated_ledger(path: Path, row_re, cutoff: datetime):
    """Date-cutoff split for a dated ledger table (gap-log.md / retrieval-log.md).

    Returns (active_lines, archived_rows). Only dated data rows older than the cutoff
    move to the archive; EVERY non-row line (titles, blockquotes, section headers,
    column headers, blanks) stays in the active file. This is deliberately stricter
    than split_log's header-bucketing: these ledgers are multi-section, so a section
    header must never migrate into the archive when its rows do.
    """
    if not path.exists():
        return [], []
...
def main():
    ...
    if archived:
        ...
        if not args.dry_run:
            write_archive(target, header, archived)
            atomic_write(CROSS_LINKS, "\n".join(active) + "\n")
    summary["cross_links"] = len(archived)

    # 5. proposals/ directory (OPT-005): move FILES whose frontmatter status is
    # executed/deferred into <qdir>/proposals/ (Path.rename — a move, not a copy).
    moved = 0
    if PROPOSALS_DIR.exists():
        for f in sorted(PROPOSALS_DIR.glob("SP-*.md")):
            text = f.read_text(errors="ignore")
            if re.search(r"^status:\s*(executed|deferred)\b", text, re.MULTILINE):
                if not args.dry_run:
                    dest_dir = qdir / "proposals"
                    dest_dir.mkdir(parents=True, exist_ok=True)
                    f.rename(dest_dir / f.name)
                moved += 1
    summary["proposals_dir"] = moved

    # 6. gap-log.md
    active, archived = split_dated_ledger(GAP_LOG, LEDGER_ROW_RE, cutoff)
    if archived:
        target = qdir / "gap-log.md"
        header = (
            "## Specification Registry: Archived Retrieval Gap Log\n\n"
            f"Rotated from `wiki/pending/gap-log.md` for {args.quarter} (rows dated before {cutoff:%Y-%m-%d}).\n\n"
        )
        if not args.dry_run:
            write_archive(target, header, archived)
            atomic_write(GAP_LOG, "\n".join(active) + "\n")
    summary["gap_log"] = len(archived)

    # 7. retrieval-log.md
    active, archived = split_dated_ledger(RETRIEVAL_LOG, LEDGER_ROW_RE, cutoff)
    if archived:
        target = qdir / "retrieval-log.md"
        header = (
            "## Specification Registry: Archived Retrieval Log\n\n"
            f"Rotated from `wiki/pending/retrieval-log.md` for {args.quarter} (rows dated before {cutoff:%Y-%m-%d}).\n\n"
        )
        if not args.dry_run:
            write_archive(target, header, archived)
            atomic_write(RETRIEVAL_LOG, "\n".join(active) + "\n")
    summary["retrieval_log"] = len(archived)

    total = sum(summary.values())
    msg = (
        f"quarter={args.quarter} proposals={summary['proposals']} "
        f"state_lines={summary['state']} log_rows={summary['log']} "
        f"cross_links={summary['cross_links']} proposals_dir={summary['proposals_dir']} "
        f"gap_log={summary['gap_log']} retrieval_log={summary['retrieval_log']} "
