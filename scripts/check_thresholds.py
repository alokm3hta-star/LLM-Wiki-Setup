...
from wikilib import count_status_rows, count_status_rows_first_table

ROOT = Path(__file__).resolve().parent.parent

# (label, path-relative-to-root, kind, threshold, suggested-action)
# kind ∈ {"lines", "status:<value>", "status1st:<value>", "proposals-pending"}
# status1st scopes the count to only the FIRST status-bearing table (the active queue),
# ignoring any permanent Historical Resolution Ledger appended after it in the same file —
# otherwise a "resolved rows" warning can never clear once that ledger exceeds threshold.
THRESHOLDS = [
    ...
    ("gap-log.md (active size)",          "wiki/pending/gap-log.md",          "lines", 200,
     "archive prior-quarter rows: `python3 scripts/rotate_archives.py <quarter>`"),
    ("retrieval-log.md (active size)",    "wiki/pending/retrieval-log.md",    "lines", 200,
     "archive prior-quarter rows: `python3 scripts/rotate_archives.py <quarter>`"),
    ("cross-links.md (resolved rows)",    "wiki/pending/cross-links.md",      "status1st:resolved", 30,
    ...
    ("proposals (pending)",               "wiki/pending/proposals",           "proposals-pending", 100,
...
def measure(path: Path, kind: str) -> int:
    ...
    if kind.startswith("status1st:"):
        target = kind.split(":", 1)[1]
        return count_status_rows_first_table(path, [target])
    if kind.startswith("status:"):
        target = kind.split(":", 1)[1]
        return count_status_rows(path, [target])
    if kind == "proposals-pending":
        return sum(1 for f in path.glob("SP-*.md")
                    if re.search(r"^status:\s*pending\b", f.read_text(errors="ignore"), re.MULTILINE))
    return 0
