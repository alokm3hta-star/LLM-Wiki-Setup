...
from wikilib import atomic_write, count_status_rows, count_status_rows_first_table
...
def count_status(path: Path, status: str) -> int:
    return count_status_rows(path, [status])


def proposals_dir_counts() -> dict:
    """pending/total counts over wiki/pending/proposals/SP-*.md frontmatter status."""
    if not PROPOSALS_DIR.exists():
        return {"pending": 0, "total": 0}
    files = list(PROPOSALS_DIR.glob("SP-*.md"))
    pending = sum(1 for f in files
                  if re.search(r"^status:\s*pending\b", f.read_text(errors="ignore"), re.MULTILINE))
    return {"pending": pending, "total": len(files)}
...
def render_snapshot(date_str: str) -> str:
    ...
    pc = proposals_dir_counts()
    out.append(f"  proposals_dir:       pending={pc['pending']} total={pc['total']}")
    out.append(f"  cross-links.md:      lines={lines(CROSS_LINKS)} bytes={bytes_(CROSS_LINKS)} "
               ...
               f"resolved_in_active_queue={count_status_rows_first_table(CROSS_LINKS, ['resolved'])} "
               f"resolved_in_historical_ledger={count_status(CROSS_LINKS, 'resolved') - count_status_rows_first_table(CROSS_LINKS, ['resolved'])}")
